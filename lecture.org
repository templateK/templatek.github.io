...
- Error Handling:
  ...
  There will be interal error state is set when executing all of the fseries functions.

  We can check these errors by ~int ferror( FILE *file )~

  the return value is either 0 on successful or non-zero value 

  + Windows: https://docs.microsoft.com/en-us/cpp/c-runtime-library/reference/fputs-fputws?view=vs-2019

  + Linux: https://linux.die.net/man/3/explain_ferror
